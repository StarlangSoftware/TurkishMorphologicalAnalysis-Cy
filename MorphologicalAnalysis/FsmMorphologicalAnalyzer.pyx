...
cdef class FsmMorphologicalAnalyzer:
    ...
    cpdef set getPossibleWords(self, MorphologicalParse morphologicalParse, MetamorphicParse metamorphicParse):
        ...
        set
            set result.
        """
        cdef bint isRootVerb, containsVerb
        cdef Transition verbTransition, transition
        cdef set result
        cdef str currentWord, verbWord, pluralWord
        cdef int pluralIndex, i
        cdef TxtWord compoundWord, currentRoot
    ...
    cpdef __addNewParsesFromCurrentParse(self, FsmParse currentFsmParse, list fsmParse, maxLengthOrSurfaceForm,
        ...
        """
        cdef State currentState
        cdef str currentSurfaceForm, tmp, surfaceForm
        cdef int maxLength
        cdef Transition currentTransition
        cdef FsmParse newFsmParse
    ...
    cpdef list __parseWord(self, list fsmParse, maxLengthOrSurfaceForm):
        ...
        cdef TxtWord root
        cdef State currentState
        cdef str currentSurfaceForm, surfaceForm
        cdef bint exists
        result = []
        if isinstance(maxLengthOrSurfaceForm, int):
            maxLength = maxLengthOrSurfaceForm
    ...
    cpdef list morphologicalAnalysisRoot(self, str surfaceForm, TxtWord root, state=None):
        """
    ...
    cpdef morphologicalAnalysis(self, sentenceOrSurfaceForm):
        ...
        cdef list result, fsmParse
        cdef int i
        cdef str originalForm, spellCorrectedForm, surfaceForm, possibleRoot
        cdef FsmParseList wordFsmParseList, fsmParseList
        cdef TxtWord newWord, word
    ...
    cpdef robustMorphologicalAnalysis(self, sentenceOrSurfaceForm):
        ...
        cdef list result
        cdef int i
        cdef str originalForm, spellCorrectedForm, surfaceForm
        cdef FsmParseList wordFsmParseList, currentParse
        cdef list fsmParse
        if isinstance(sentenceOrSurfaceForm, Sentence):
            sentence = sentenceOrSurfaceForm
            result = []
